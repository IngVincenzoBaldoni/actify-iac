...
function ProfileCard({
  ...
  async function handleMerge(checked: boolean) {
    // Find the other profile to merge with (only works when exactly 2 profiles — Deployer case)
...
export default function LiteracySystemPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setLitStatus(typed.literacy_status);
    } catch { /* silent */ }
    setLoading(false);
  }, [systemId]);
