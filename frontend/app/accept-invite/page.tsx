...
import { doSignOut, doSignIn } from '@/lib/auth';
...
function AcceptInviteContent() {
  ...
  useEffect(() => {
    if (!cid || !t) { setStage('error'); setErrorMsg('Link non valido.'); return; }
    api.auth.validateInvite(cid, t)
      .then(data => {
        setEmail(data.email);
      ...
      .catch((err: { message?: string; statusCode?: number }) => {
        setErrorMsg(err.message ?? 'Invito non valido o scaduto.');
        setStage('error');
      });
  }, [cid, t]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSubmitting(true);
    try {
      await api.auth.acceptInvite({ company_id: cid, token: t, password });
      // Force-clear any existing session, then sign in as the new user
        ...
        <div style={{
          ...
          {stage === 'loading' && (
            <div style={{ textAlign: 'center', padding: '20px 0' }}>
              ...
              <p style={{ color: 'var(--muted)', fontSize: 14 }}>Verifica invito…</p>
            </div>
          )}
          ...
          {stage === 'form' && (
            <>
              ...
              <p style={{ color: 'var(--muted)', fontSize: 13, margin: '0 0 28px', lineHeight: 1.5 }}>
                Sei stato invitato a collaborare su <strong style={{ color: 'var(--text2)' }}>{companyName}</strong>.<br/>
          ...
          {stage === 'success' && (
            <div style={{ textAlign: 'center' }}>
              ...
              <p style={{ color: 'var(--muted)', fontSize: 14, lineHeight: 1.6, marginBottom: 28 }}>
                Benvenuto su Actify. Accedi ora con la tua email e la password che hai scelto.
              </p>
              <a
                href="/login"
                style={{
